...
Homey app to receive signals from Byron BY doorbell buttons; and to send signal to Byron BY doorbell chimes.
...
* melodyNr:
Contains the melody number of the melody that was chosen. This is a number between 1 and 8. These are the numbers that are listed in the manual that comes with the device. Some chimes only support 4 melodies. Some chimes support only 6 melodies.
* melodyId:
...
* melodyNr:
Contains the melody that should be played by the bell. You can select the melody from a dropdown list. When you select the proper card, the list of melodies should exactly match with what is possible. This parameter is not available for chimes that have only one melody.
* melodyId:
